...
- Using `SolaceBinderHeaders.LARGE_MESSAGE_SUPPORT` header to enable chunking per message
...
```
  ...
  Generates a 1 MB string payload, sets a 30 second TTL, keeps the message DMQ-eligible, and enables `SolaceBinderHeaders.LARGE_MESSAGE_SUPPORT`. The binder then automatically splits the payload into multiple chunk messages before sending them to the broker.
...
1. The publisher sets `SolaceBinderHeaders.LARGE_MESSAGE_SUPPORT` to `true` on a 1 MB payload.
