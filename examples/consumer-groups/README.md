...
```
  ...
  Publishes a message every 2 seconds to `example/groups/topic` using a scheduled task. The sample also sets a 30 second TTL and `solace_dmqEligible=true` so durable-group traffic stays aligned with the recommended header defaults.
...
```
INFO  Durable consumer received: group-msg-1
INFO  Anonymous consumer received: group-msg-1
```
