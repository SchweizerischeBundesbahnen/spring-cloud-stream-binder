...
public class FlowReceiverContainer {
    ...
    public UUID bind() throws JCSMPException {
        ...
        try {
            ...
            if (existingFlowReceiverReference != null) {
                ...
                final ConsumerFlowProperties flowProperties = new ConsumerFlowProperties().setEndpoint(JCSMPFactory.onlyInstance().createQueue(queueName)).setAckMode(JCSMPProperties.SUPPORTED_MESSAGE_ACK_CLIENT).setStartState(!isPaused.get());
                ...
                if (logger.isInfoEnabled()) {
                    if (flowReceiver instanceof FlowHandleImpl handler) {
                        // since for some reason solace adds 2097152 in Binary (1000000000000000000000) we need to filter it away and only keep the real flowId.
                        long flowId = handler.getFlowId() & (0b111111111111111111111);
                        logger.info(String.format("Flow with FlowId=%d receiver container %s started in state '%s'", flowId, id, isPaused.get() ? "Paused" : "Running"));
                    } else {
    ...
    public MessageContainer receive(Integer timeoutInMillis) throws JCSMPException, UnboundFlowReceiverContainerException {
        ...
        return new MessageContainer(xmlMessage, flowReceiverReference.getId(), flowReceiverReference.getStaleMessagesFlag());
    }
