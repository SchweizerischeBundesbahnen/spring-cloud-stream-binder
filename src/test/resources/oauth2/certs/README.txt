# Usage
If you use localdev having docker and docker-compose installed, you are good to go.
How-ever if you are using external docker host, run with bash (on windows gitbash):
```
cd <git root dir>
./create_trust_store_for_externalized_docker.bash
```
It will create a truststore with signed certificates for the host given in DOCKER_HOST environment variable.
...
openssl req -x509 -new -nodes -key ./rootCA/rootCA.key -sha256 -days 7300 -out ./rootCA/rootCA.crt -subj "/C=CA/ST=Ontario/L=Kanata/O=Solace Systems/CN=Root CA"
...
openssl x509 -req -in ./broker/solbroker.csr -CA ./rootCA/rootCA.crt -CAkey ./rootCA/rootCA.key -CAcreateserial -out ./broker/solbroker.crt -days 7300 -sha256 -extensions req_ext -extfile ./solbroker_san.conf
...
2. (changed) Create a CSR (Certificate Signing Request) for the client:
openssl req -new -key ./client/client.key -out ./client/client.csr -config ./client_san.conf


3. Sign the client CSR with your Root CA to generate the client certificate valid for 20 years:
openssl x509 -req -in ./client/client.csr -CA ./rootCA/rootCA.crt -CAkey ./rootCA/rootCA.key -CAcreateserial -out ./client/client.crt -days 7300 -sha256
...
openssl x509 -req -in ./keycloak/keycloak.csr -CA ./rootCA/rootCA.crt -CAkey ./rootCA/rootCA.key -CAcreateserial -out ./keycloak/keycloak.crt -days 7300 -sha256 -extensions req_ext -extfile ./keycloak_san.conf
