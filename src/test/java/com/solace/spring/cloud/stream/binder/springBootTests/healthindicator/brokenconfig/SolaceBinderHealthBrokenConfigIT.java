...
@ActiveProfiles(profiles = {"broken-queue-subscription"})
@ExtendWith(OutputCaptureExtension.class)
@DirtiesContext
public class SolaceBinderHealthBrokenConfigIT {
    ...
    void applyBrokenConfig_healthShouldBeDownAndKeepDownAfterAtLeast3Retries(CapturedOutput output) throws Exception {
        ...
        await().pollInterval(Duration.ofSeconds(1))
                .atMost(10, TimeUnit.MINUTES).until(() -> ERROR_BREAKING_BINDING.matcher(output.getOut()).results().count() >= 3);

        String content = client.send(request, HttpResponse.BodyHandlers.ofString()).body();
        assertEquals("DOWN", objectMapper.readTree(content).get("status").asText());
    }
}
